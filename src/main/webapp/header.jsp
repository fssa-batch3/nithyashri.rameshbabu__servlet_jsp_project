...
<header class="header">
	...
	<c:if test="${empty sessionScope.loggedInEmail}">
		...
		<nav class="navbar">
			<a href="<%=admin%>">Admin</a> <a href="<%=contact%>">Doctor</a>
	...
	<c:if test="${not empty sessionScope.loggedInEmail}">
		...
		<nav class="navbar">
			...
			<!-- <a href=blogs>Blogs</a> --> <a
				...
				history</a> <a href="./LogoutServlet" onclick="logout_link()">logout</a><img
				src="${user.profilePic}" onclick="profile()" id="random_img"
				alt="img">
		</nav>
		<div id="menu-btn" class="fas fa-bars"></div>
	</c:if>
